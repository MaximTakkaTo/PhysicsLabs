    ...
    static class Calc
    {
        ...
        public static void Out(double[] data, double h, double Ep1, double Ep2, out string cH, out string cEp1, out string cEp2, out string cTbl)
        {
            ...
            if ((Math.Abs(Ep1 - Ep2) / Ep1 * 100) <= 13)
                cTbl = "Выполняя лабораторную работу мы научились измерять потенциальную энергию поднятого над землей тела и упруго деформированной пружины. Сравнивавя полученные потенциальные энергии Ep1 = " + Ep1.ToString("0.###") + " и Ep2 = " + Ep2.ToString("0.###") + " ,убеждаемся ,что они примерно одинаковы.Это подтверждает закон сохранения энергии.";
            else
